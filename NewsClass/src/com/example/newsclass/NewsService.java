...
public class NewsService extends IntentService  {
	...
	private void checkForNews() {
		...
		while(classesSelectedCursor.moveToNext()){
			...
			String className = classesSelectedCursor.getString(classesSelectedCursor.getColumnIndex("fullname"));
			
			NewItem[] result = _requests.requestNews(classId,className);
			int countNews = 0;
	...
	private void launchNotification(String className, int countNews) {
		...
		rv.setTextViewText(R.id.textView1, "Class" + className + ": " + countNews + "News added");
